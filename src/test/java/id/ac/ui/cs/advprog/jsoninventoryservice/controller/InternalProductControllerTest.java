...
class InternalProductControllerTest {
    ...
    void reserveStock_Success() throws Exception {
        ...
        StockReserveRequest req = new StockReserveRequest(); req.setQuantity(2); req.setOrderId(id);
        ProductResponse res = ProductResponse.builder().id(id).build();
        when(stockService.reserveStock(eq(id), any())).thenReturn(Optional.of(res));

        mockMvc.perform(post("/internal/products/{id}/stock/reserve", id)
    ...
    void reserveStock_Fail() throws Exception {
        UUID id = UUID.randomUUID();
        StockReserveRequest req = new StockReserveRequest(); req.setQuantity(2);
        when(stockService.reserveStock(eq(id), any())).thenReturn(Optional.empty());
    ...
    void processPostOrder_BadRequest() throws Exception {
        ...
        when(stockService.processPostOrder(eq(id), any(PostOrderRequest.class)))
                .thenThrow(new IllegalArgumentException("Action tidak valid"));
